...
#define AD9545_NCO_CYCLE_TYPE_MSK		BIT(1)
#define AD9545_NCO_DELTA_TYPE_MSK		BIT(0)
...
static int ad9545_sys_clk_read_setup(struct ad9545_state *st)
{
	...
	ret = regmap_bulk_read(st->regmap, AD9545_SYS_CLK_STABILITY_MS,
			       &regval, 3);
	if (ret < 0)
		return ret;
...
static int ad9545_get_r_div(struct ad9545_state *st, int addr, u32 *r_div)
{
	...
	if (ret < 0)
		return ret;
	div = le32_to_cpu(regval) & 0x3fffffff;
...
ad9545_calc_pll_zero_delay_params(struct ad9545_pll_clk *clk,
				  unsigned long rate,
...
static unsigned long ad9545_calc_pll_params(struct ad9545_pll_clk *clk,
					    unsigned long rate,
					    unsigned long parent_rate, u32 *m,
					    u64 *n_hitless, u64 *n_buildout,
					    unsigned long *frac,
					    unsigned long *mod, int profile)
{
	u32 min_dpll_n_div;
	u64 output_rate;
	u32 dpll_n_div;
...
static const char *ad9545_source_name(int source)
{
	...
	case 2:
	case 3:
		source_name = ad9545_ref_clk_names[source];
		break;
	case AD9545_SOURCE_FEEDBACK_FROM_DPLL0:
		source_name = "Feedback from DPLL0";
		break;
	case AD9545_SOURCE_FEEDBACK_FROM_DPLL1:
		source_name = "Feedback from DPLL1";
		break;
	case AD9545_SOURCE_AUX_NCO0:
		source_name = ad9545_aux_nco_clk_names[0];
		break;
	case AD9545_SOURCE_AUX_NCO1:
		source_name = ad9545_aux_nco_clk_names[1];
		break;
	default:
		source_name = "Unknown";
		break;
					   ...
					   bool *freerun, bool *holdover)
{
	...
	if (!active_profile || !freerun || !holdover)
		return -EINVAL;
...
static int ad9545_pll_set_parent(struct clk_hw *hw, u8 index)
{
	struct ad9545_pll_clk *pll = to_pll_clk(hw);
	struct ad9545_state *st = pll->st;
	int profile, active_profile, source;
	int i, count;
	u32 val;
	int ret;

	for (i = 0; i < AD9545_MAX_DPLL_PROFILES; i++) {
		if (!pll->profiles[i].en)
			dev_info(st->dev, "PLL%d profile %d is disabled\n", pll->address, i);
		else
			dev_info(st->dev, "PLL%d profile %d has source %s\n",
				 pll->address, i, ad9545_source_name(pll->profiles[i].source));
		if (pll->profiles[i].en &&
		    pll->profiles[i].parent_index == index)
			break;
	}

	if (i == AD9545_MAX_DPLL_PROFILES) {
		dev_info(st->dev, "PLL%d has no profile for parent index %d\n",
			 pll->address, index);
		return -EINVAL;
	}

	profile = i;
	source = pll->profiles[i].source;

	/* switch to the profile */

	dev_info(st->dev, "switching PLL%d active profile to %d (%s) ...\n",
		 pll->address, profile, ad9545_source_name(source));
...
static unsigned long ad9545_pll_clk_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	...
	ret = ad9545_pll_get_operation_status(hw, &dummy,
					      &freerun, &holdover);
	if (ret < 0)
		return 0;
	if (freerun) {
	...
	for (j = 0; j < AD9545_MAX_DPLL_PROFILES; j++) {
		...
		if (parent_rate == clk_hw_get_rate(clk->parents[parent_index]))
			break;
	}

	if (j == AD9545_MAX_DPLL_PROFILES) {
		dev_err(clk->st->dev,
			"PLL%d doesn't have a parent clk whose clk_rate is %lu\n",
			i, parent_rate);
		return -EINVAL;
	}

	if (clk->profiles[j].hitless) {
		int output_nr;
		u32 q_div;

		if (clk->profiles[j].external_zero_delay) {
			dev_err(clk->st->dev, "no support for external zero delay yet\n");
			return -EINVAL;
		}

		output_nr = clk->profiles[j].internal_zero_delay_fb_path;
		...
		if (clk->profiles[j].tag_mode == AD9545_TAG_NONE) {
			...
			n += 1;
		} else if (clk->profiles[j].tag_mode == AD9545_TAG_FEEDBACK_PATH) {
			unsigned int r;
		...
		output_rate = mod == 0 ? 0 : mul_u64_u32_div(frac * parent_rate, m, mod);
...
static int ad9545_pll_set_feedback_tagging(struct ad9545_pll_clk *clk,
					   unsigned long rate,
					   unsigned long parent_rate,
					   unsigned int profile, u32 m,
					   u64 *n_buildout, u64 *n_hitless)
{
	unsigned long out_rate_hz;
	...
	ret = regmap_update_bits(clk->st->regmap,
				 AD9545_DPLLX_FB_MODE(clk->address, profile),
				 AD9545_LOOP_FILTER_BASE_MSK |
				 AD9545_TAG_MODE_MSK, val);
	if (ret < 0)
		return ret;
...
static int ad9545_pll_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long parent_rate)
{
	...
	for (j = 0; j < AD9545_MAX_DPLL_PROFILES; j++) {
		...
		if (hitless && !external_zero_delay && parent_rate < 2000) {
			ret = ad9545_pll_set_feedback_tagging(clk, rate,
							      parent_rate, j,
							      m, &n_hitless,
							      &n_buildout);
...
static int ad9545_pll_read_profile(struct ad9545_pll_clk *pll, int index,
				   struct ad9545_dpll_profile *profile)
{
	struct ad9545_state *st = pll->st;
	__le32 regval;
	...
	ret = regmap_read(st->regmap, AD9545_DPLLX_EN(i, j), &val);
	if (ret < 0)
		return ret;
	profile->en = (val & AD9545_EN_PROFILE_MSK) ? 1 : 0;
	profile->priority = FIELD_GET(AD9545_SEL_PRIORITY_MSK, val);

	ret = regmap_read(st->regmap, AD9545_DPLLX_SOURCE(i, j), &val);
	if (ret < 0)
		return ret;
	profile->source = val;

	ret = regmap_read(st->regmap, AD9545_DPLLX_FB_PATH(i, j), &val);
	if (ret < 0)
		return ret;
	profile->internal_zero_delay_fb_path = val;

	ret = regmap_read(st->regmap, AD9545_DPLLX_FB_MODE(i, j), &val);
	if (ret < 0)
		return ret;
	profile->hitless = FIELD_GET(AD9545_EN_HITLESS_MSK, val);
	profile->external_zero_delay = FIELD_GET(AD9545_EN_EXT_ZERO_DELAY_MSK,
						 val);
	profile->tag_mode = FIELD_GET(AD9545_TAG_MODE_MSK, val);
	profile->loop_filter_base = FIELD_GET(AD9545_LOOP_FILTER_BASE_MSK, val);

	ret = regmap_bulk_read(st->regmap, AD9545_DPLLX_LOOP_BW(i, j),
			       &regval, 4);
	if (ret < 0)
		return ret;
	profile->loop_bw_uhz = le32_to_cpu(regval);

	ret = regmap_bulk_read(st->regmap, AD9545_DPLLX_HITLESS_N(i, j),
			       &regval, 4);
	if (ret < 0)
		return ret;
	profile->hitless_n_div = le32_to_cpu(regval);

	ret = regmap_bulk_read(st->regmap, AD9545_DPLLX_N_DIV(i, j),
			       &regval, 4);
	if (ret < 0)
		return ret;
	profile->buildout_n_div = le32_to_cpu(regval);

	ret = regmap_bulk_read(st->regmap, AD9545_DPLLX_FRAC_DIV(i, j),
			       &regval, 3);
	if (ret < 0)
		return ret;
	profile->buildout_frac = le32_to_cpu(regval);

	ret = regmap_bulk_read(st->regmap, AD9545_DPLLX_MOD_DIV(i, j),
			       &regval, 3);
	if (ret < 0)
		return ret;
	profile->buildout_mod = le32_to_cpu(regval);

	ret = regmap_read(st->regmap, AD9545_DPLLX_FAST_L1(i, j), &val);
	if (ret < 0)
		return ret;
	profile->fast_acq_excess_bw = val;

	ret = regmap_read(st->regmap, AD9545_DPLLX_FAST_L2(i, j), &val);
	if (ret < 0)
		return ret;
	profile->fast_acq_settle_ms = val;
	profile->fast_acq_timeout_ms = val >> 4;
...
#ifdef CONFIG_DEBUG_FS
static void ad9545_pll_show_profile(struct seq_file *s, int pll, int index,
				    struct ad9545_dpll_profile *profile)
{
	const char *ref_source_name;
	...
	ref_source_name = ad9545_source_name(profile->source);
	seq_printf(s, "  Ref source: %s\n", ref_source_name);

	seq_printf(s, "  Feedback mode: %s\n",
		   profile->hitless ? "Hitless" : "Buildout");

	if (profile->hitless) {
		const char *fb_path_name = "Unknown";
		int fb_path;

		seq_printf(s, "  Zero delay: %s\n",
			   profile->external_zero_delay ?
			   "External" : "Internal");
		seq_printf(s, "  Tag mode: %u\n", profile->tag_mode);
		seq_printf(s, "  Loop filter base: %s\n",
			   profile->loop_filter_base ?
			   "High phase margin" : "Nominal phase margin");

		fb_path = profile->internal_zero_delay_fb_path;
		if (profile->external_zero_delay) {
			if (fb_path < ARRAY_SIZE(ad9545_ref_clk_names))
				fb_path_name = ad9545_ref_clk_names[fb_path];
		} else {
			fb_path += 6 * pll;
			if (fb_path < ARRAY_SIZE(ad9545_out_clk_names))
				fb_path_name = ad9545_out_clk_names[fb_path];
		}
		seq_printf(s, "  Internal zero delay feedback path: %s\n",
			   fb_path_name);
	}

	seq_printf(s, "  Loop bandwidth: %u uHz\n",
		   profile->loop_bw_uhz);

	seq_printf(s, "  Hitless N-divider: %llu\n",
		   (u64)profile->hitless_n_div + 1);
	seq_printf(s, "  Buildout N-divider: %llu\n",
		   (u64)profile->buildout_n_div + 1);

	if (!profile->hitless) {
		seq_printf(s, "  Buildout fraction: %u\n",
			   profile->buildout_frac);
		seq_printf(s, "  Buildout modulus: %u\n",
			   profile->buildout_mod);
	}

	seq_printf(s, "  Fast acquisition excess bandwidth: %u\n",
		   ad9545_fast_acq_excess_bw_map[profile->fast_acq_excess_bw]);
	seq_printf(s, "  Fast acquisition settle: %u msec\n",
		   ad9545_fast_acq_time_map[profile->fast_acq_settle_ms]);
	seq_printf(s, "  Fast acquisition timeout: %u msec\n",
		   ad9545_fast_acq_time_map[profile->fast_acq_timeout_ms]);
}
...
static int ad9545_pll_clk_debugfs_show(struct seq_file *s, void *p)
{
	...
	ret = regmap_read(st->regmap, AD9545_PLLX_STATUS(i), &val);
	if (ret < 0)
		return ret;

	seq_printf(s, "PLL status: %s\n",
		   (val & AD9545_PLL_LOCKED) ? "Locked" : "Unlocked");

	ret = regmap_read(st->regmap, AD9545_PLLX_OPERATION(i), &val);
	if (ret < 0)
		return ret;

	seq_printf(s, "Freerun Mode: %s\n",
		   (val & AD9545_PLL_FREERUN) ? "On" : "Off");
	seq_printf(s, "Holdover Mode: %s\n",
		   (val & AD9545_PLL_HOLDOVER) ? "On" : "Off");
	seq_printf(s, "PLL: %s\n",
		   (val & AD9545_PLL_ACTIVE) ? "Active" : "Inactive");
	seq_printf(s, "Profile Number: %ld\n",
		   FIELD_GET(AD9545_PLL_ACTIVE_PROFILE, val));

	ret = regmap_read(st->regmap, AD9545_APLLX_M_DIV(pll->address), &val);
	if (ret < 0)
		return ret;
	seq_printf(s, "M-divider: %u\n", val);

	ret = regmap_bulk_read(st->regmap, AD9545_TEMP0, &regval16, 2);
	if (ret < 0)
		return ret;

	temp = le16_to_cpu(regval16);
	temp /= (1 << 7);
	...
	for (j = 0; j < AD9545_MAX_DPLL_PROFILES; j++) {
		...
		ret = ad9545_pll_read_profile(pll, j, &profile);
		if (ret < 0)
			return ret;

		ad9545_pll_show_profile(s, i, j, &profile);
	}
...
static const struct clk_ops ad9545_pll_clk_ops = {
	...
	.set_parent = ad9545_pll_set_parent,
				      ...
				      u64 *freq)
{
	...
	ret = regmap_bulk_read(st->regmap, AD9545_NCOX_CENTER_FREQ(addr),
			       &regval64, 7);
	if (ret < 0)
		return ret;

	*freq = le64_to_cpu(regval64) & GENMASK_ULL(55, 0);

	return 0;
}

static int ad9545_set_nco_center_freq(struct ad9545_state *st, int addr,
				      u64 freq)
{
	__le64 regval64;
	...
	return regmap_bulk_write(st->regmap, AD9545_NCOX_CENTER_FREQ(addr),
				 &regval64, 7);
}

static int ad9545_get_nco_offset_freq(struct ad9545_state *st, int addr,
				      u32 *freq)
{
	__le32 regval;
	int ret;

	ret = regmap_bulk_read(st->regmap, AD9545_NCOX_OFFSET_FREQ(addr),
			       &regval, 4);
	if (ret < 0)
		return ret;
...
static int ad9545_set_nco_offset_freq(struct ad9545_state *st, int addr,
				      u32 freq)
{
	__le32 regval;
	...
	return regmap_bulk_write(st->regmap, AD9545_NCOX_OFFSET_FREQ(addr),
				 &regval, 4);
}
...
static int ad9545_get_nco_freq(struct ad9545_state *st, int addr, u64 *freq)
{
	...
	*freq = center_freq + ((u64)offset_freq << (40 - 24));
...
int ad9545_get_aux_nco_tuning_freq(struct clk *clk, u64 *freq)
{
	...
	dev_dbg(nco->st->dev, "nco%d: get tuning frequency: %llu\n",
		nco->address, *freq);
...
int ad9545_set_aux_nco_tuning_freq(struct clk *clk, u64 freq)
{
	...
	dev_dbg(nco->st->dev, "nco%d: set tuning frequency: %llu\n",
		nco->address, freq);
...
static int ad9545_get_nco_freq_hz(struct ad9545_state *st, int addr, u32 *freq)
{
	...
	*freq = DIV_ROUND_CLOSEST(val64, 1ULL << 40);
...
static int ad9545_set_nco_freq_hz(struct ad9545_state *st, int addr, u32 freq)
{
	...
	if (freq <= 0xffff) {
		ret = ad9545_set_nco_center_freq(st, addr, (u64)freq << 40);
		if (ret < 0)
			return ret;

		ret = ad9545_set_nco_offset_freq(st, addr, 0);
		if (ret < 0)
			return ret;
	} else {
		ret = ad9545_set_nco_center_freq(st, addr, 0xffff);
		if (ret < 0)
			return ret;
		...
		ret = ad9545_set_nco_offset_freq(st, addr, freq << 24);
		if (ret < 0)
			return ret;
	}
...
static int ad9545_plls_post_setup(struct ad9545_state *st)
{
	...
	for (i = 0; i < ARRAY_SIZE(ad9545_pll_clk_names); i++) {
		...
		for (j = 0; j < AD9545_MAX_DPLL_PROFILES; j++) {
			struct ad9545_dpll_profile *p = &pll->profiles[j];

			if (p->source != AD9545_SOURCE_AUX_NCO0 &&
			    p->source != AD9545_SOURCE_AUX_NCO1)
				continue;

			k = p->source - AD9545_SOURCE_AUX_NCO0;
			freq_int = st->aux_nco_clks[k].rate_requested_hz;
			if (freq_int == 0) {
				/*
				 * if no requested rate and the profile is
				 * already enabled, we don't need to change
				 * anything
				 */
				if (p->en)
					continue;

				/* otherwise, we need a requested rate to set
				 * it up.
				 */
				dev_err(st->dev,
					"missing requested rate for AUX NCO%d\n", k);
				return -EINVAL;
			}

			*p = pll->profiles[active_profile];
			p->source = k + AD9545_SOURCE_AUX_NCO0;
...
static int ad9545_post_setup(struct ad9545_state *st)
{
	...
	if (num > 0) {
		...
		ret = fwnode_property_read_u32_array
				(fwnode, "adi,set-aux-nco-frequencies",
				 frequencies, num);
		if (ret < 0)
			return ret;
	...
	if (num > 0) {
		...
		ret = fwnode_property_read_u32_array
				(fwnode, "adi,add-dpll-aux-nco-profiles",
				 profiles, num);
		if (ret < 0)
			return ret;
