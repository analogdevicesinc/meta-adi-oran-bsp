...
static int eth_etile_tx_rx_user_flow(struct intel_fpga_etile_eth_private *priv)
{
	...
	/* Adjust UI value when PTP is enabled */
	if (priv->ptp_enable) {
...
#else
		netdev_err(priv->dev, "%s: detecting speed not implemented\n", __func__);
#endif
	} else {
...
#define ADI_BRIDGE_MAC_OIF_RESET_TX		BIT(16)
#define ADI_BRIDGE_MAC_OIF_RESET_RX		BIT(20)  // Not used
...
#ifdef CONFIG_INTEL_FPGA_ETILE_LITE
	if (priv->ethernet && priv->ethernet->link_down)
		priv->ethernet->link_down(priv->ethernet->dev);

	if (priv->eth_ctrl) {
		u32 value = readl(priv->eth_ctrl);
		writel(value | ADI_BRIDGE_MAC_OIF_RESET_TX, priv->eth_ctrl);
	}
#endif
					 ...
					 bool rx_pause)
{
	...
	if (priv->ethernet && priv->ethernet->link_up)
		priv->ethernet->link_up(priv->ethernet->dev);
#endif
}
...
#ifdef CONFIG_INTEL_FPGA_ETILE_LITE
void intel_fpga_etile_set_ethernet(struct net_device *dev, const struct intel_fpga_etile_lite_ethernet *ethernet)
{
	struct intel_fpga_etile_eth_private *priv = netdev_priv(dev);
	priv->ethernet = ethernet;
}
EXPORT_SYMBOL_GPL(intel_fpga_etile_set_ethernet);
#endif
...
static int intel_fpga_etile_probe(struct platform_device *pdev)
{
...
#else
	if (!of_id) {
		dev_err(&pdev->dev, "Unknown device\n");
		ret = -ENODEV;
	...
	priv->ptp_enable = of_property_read_bool(pdev->dev.of_node,
						 "altr,has-ptp");
	if (priv->ptp_enable)
		dev_info(&pdev->dev, "\tPTP is enabled\n");
	else
		dev_info(&pdev->dev, "\tPTP is disabled\n");
#ifndef CONFIG_INTEL_FPGA_ETILE_LITE
	if (priv->ptp_enable &&
...
#ifdef CONFIG_INTEL_FPGA_ETILE_LITE
	priv->eth_ctrl = devm_platform_ioremap_resource_byname(pdev, "eth_ctrl");
	if (IS_ERR(priv->eth_ctrl)) {
		dev_err(&pdev->dev, "Failed to remap eth_ctrl\n");
		goto err_free_netdev;
	}
#endif
...
#else
	/* Hack for Analog Devices Kerberos board
	 * The Kerberos board has only 0x4000 memory range for each E-Tile.
	 * So it uses paged memory access. We embed the memory region
	 * information in the lowest 2 bits of address.
	 */
	if ((uintptr_t)priv->mac_dev & ADI_KERBEROS_ETILE_HACK_MASK) {
		dev_err(&pdev->dev, "mac_dev not aligned at 4 bytes\n");
		ret = -EINVAL;
		goto err_free_netdev;
	}

	priv->rsfec = (void __iomem *)priv->mac_dev + ADI_KERBEROS_RSFEC_BIT;
	priv->xcvr = (void __iomem *)priv->mac_dev + ADI_KERBEROS_XCVR_BIT;
#endif
