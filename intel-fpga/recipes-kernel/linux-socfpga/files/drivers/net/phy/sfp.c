...
#ifdef CONFIG_WA_MULTIPLE_SFPS_ON_ONE_I2C_BUS
/* Both QSFP cages on ADI Kerberos are connected to the same I2C bus.
 * This causes a conflict issue. This workaround uses a mutex to
 * ensure only one QSFP is enabled each time.
 */
static DEFINE_MUTEX(sfp_wa_mutex);
#endif
...
enum {
	...
	SFP_F_QSFP_SELECT = BIT(GPIO_QSFP_SELECT),
...
static const struct sff_data qsa_data = {
	.gpios = SFP_F_PRESENT | SFP_F_QSFP_SELECT,
	.module_supported = sfp_module_supported,
};
...
static const struct of_device_id sfp_of_match[] = {
	...
	{ .compatible = "sff,qsa", .data = &qsa_data, },
	{ },
};
...
static void sfp_gpio_set_state(struct sfp *sfp, unsigned int state)
{
	if (state & SFP_F_PRESENT) {
		...
		if (sfp->gpio[GPIO_QSFP_SELECT])
			gpiod_direction_output(sfp->gpio[GPIO_QSFP_SELECT],
					       state & SFP_F_QSFP_SELECT);
	} else {
...
#ifdef CONFIG_WA_MULTIPLE_SFPS_ON_ONE_I2C_BUS
	mutex_lock(&sfp_wa_mutex);
#endif

	if (sfp->gpio[GPIO_QSFP_SELECT])
		sfp->set_state(sfp, sfp->state | SFP_F_QSFP_SELECT);

	ret = sfp->read(sfp, a2, addr, buf, len);

	if (sfp->gpio[GPIO_QSFP_SELECT])
		sfp->set_state(sfp, sfp->state);

#ifdef CONFIG_WA_MULTIPLE_SFPS_ON_ONE_I2C_BUS
	mutex_unlock(&sfp_wa_mutex);
#endif
...
static int sfp_write(struct sfp *sfp, bool a2, u8 addr, void *buf, size_t len)
{
	...
	ret = sfp->write(sfp, a2, addr, buf, len);
...
static int sfp_probe(struct platform_device *pdev)
{
	...
	/* Always de-select the module initially */
	if (sfp->gpio[GPIO_QSFP_SELECT])
		sfp->state &= ~SFP_F_QSFP_SELECT;
	...
	if (sfp->state & SFP_F_PRESENT) {
		...
		dev_info(sfp->dev, "sfp is not present\n");
	}
