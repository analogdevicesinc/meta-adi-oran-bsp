 ...
 *
 * Copyright 2014,2023 Analog Devices, Inc.
 *
...
#define AD9528_PLL1_CHARGE_PUMP_AUTO_TRISTATE_DIS	BIT(13)
...
#define AD9528_SYSREF_PATTERN_CLK_SRC_PLL1	BIT(4)
#define AD9528_SYSREF_RESAMPLER_CLK_SRC_PLL1	BIT(3)
...
static int ad9528_read(struct iio_dev *indio_dev, unsigned addr)
{
	...
	struct spi_transfer t[] = {
		{
			.tx_buf = tx_buf,
			.rx_buf = rx_buf,
			.len = 2 + AD9528_TRANSF_LEN(addr),
		}
	};

	cmd = AD9528_READ | AD9528_CNT(AD9528_TRANSF_LEN(addr)) | AD9528_ADDR(addr);
	tx_buf[0] = (cmd >> 8) & 0xff;
	tx_buf[1] = cmd & 0xff;
	...
	val = 0;
	for (i = 0; i < AD9528_TRANSF_LEN(addr); i++) {
		val <<= 8;
		val |= rx_buf[i + 2];
	}
...
static int ad9528_write(struct iio_dev *indio_dev, unsigned addr, unsigned val)
{
	...
	cmd = AD9528_WRITE | AD9528_CNT(AD9528_TRANSF_LEN(addr)) | AD9528_ADDR(addr);
	tx_buf[0] = (cmd >> 8) & 0xff;
	tx_buf[1] = cmd & 0xff;

	for (i = 0; i < AD9528_TRANSF_LEN(addr); i++)
		tx_buf[2 + i] = val >> (AD9528_TRANSF_LEN(addr) - 1 - i) * 8;
